...
function printReport(r: CalibrationReport, leadsRequested: number): void {
  ...
  if (r.autoApproved > 0) {
    console.log(`    auto-approved        ${r.autoApproved}   (no review step — clear to send)`);
  }
  if (r.heldForNoContact > 0) {
    console.log(`    held — no contact    ${r.heldForNoContact}   (no email or verified mobile — not synced to GHL)`);
  }
