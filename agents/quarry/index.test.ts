...
describe("Quarry — quarry_run_discovery", () => {
  it("builds location-specific searches and runs discovery synced to GHL", async () => {
    ...
    vi.mocked(chatWithTools)
      .mockResolvedValueOnce({
        content: [toolUseBlock("c1", "quarry_run_discovery", { location: "Ottawa, ON", maxLeads: 15 })],
        stop_reason: "tool_use",
      } as any)
      .mockResolvedValueOnce(endTurn("Found 40 in Ottawa, 12 qualified, 9 auto-approved."));

    const reply = await quarryAgent.generateReply("k8", "find 15 businesses in Ottawa");
  ...
  it("hard-caps maxLeads at 15 server-side no matter what's requested — confirmed live, a bigger batch crashed the shared instance", async () => {
    ...
    expect(pipelineMocks.run).toHaveBeenCalledWith(expect.objectContaining({ maxLeads: 15 }));
  });

  it("defaults to 15 (not a larger number) when maxLeads is omitted", async () => {
    pipelineMocks.run.mockResolvedValue({
      discovered: 1, qualified: 1, autoApproved: 1, heldForNoContact: 0, syncedToGhl: 1, withEmail: 1, errors: [],
    });
    vi.mocked(chatWithTools)
